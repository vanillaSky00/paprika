...
class MoveInput(BaseModel):
    location_id: str = Field(description="The ID of the location or object to move to (e.g., 'Kitchen', 'Stove_01').")

class InteractInput(BaseModel):
    target_id: str = Field(description="The ID of the object to interact with.")
    interaction_type: str = Field(description="Type of interaction: 'use', 'pick_up', 'open', 'close'.", default="use")

class SayInput(BaseModel):
    text: str = Field(description="The sentence to speak out loud.")

class ThinkInput(BaseModel):
    thought: str = Field(description="The content of your internal monologue. No one else hears this.")
...
class InteractToolBuilder(BaseToolBuilder):
    def build(self, context: ToolContext) -> StructuredTool:
        def interact_logic(target_id: str, interaction_type: str = "use"):
            return {"status": "interacting", "target": target_id, "type": interaction_type}
